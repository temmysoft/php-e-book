...
echo "              <tr><td class=table_rows height=25 width=20% style='padding-left:32px;' nowrap>Nacimiento:</td><td colspan=2 width=80%
                      style='color:red;font-family:Tahoma;font-size:11px;padding-left:20px;'><input type='text' 
...
if($_POST['submit']=='Aplicar Cambios'){
	...
	if(mysql_num_rows($res)<=0){
		...
		echo "<BR>";
		echo "            <table align=center class=table_border width=40% border=0 cellpadding=3 cellspacing=0>\n";
		echo "              <tr><td class=table_rows align=center colspan=3 style='color:green;font-family:Tahoma;font-size:12px;'>Se actualizo la persona con exito !!!</td></tr>\n";
		echo "            </table>\n";
	}else {
		echo "<BR>";
		echo "            <table align=center class=table_border width=40% border=0 cellpadding=3 cellspacing=0>\n";
		echo "              <tr><td class=table_rows align=center colspan=3 style='color:red;font-family:Tahoma;font-size:12px;'>ERROR: La persona ya existe !!!</td></tr>\n";
		echo "            </table>\n";
	}
...
if($_POST['submit']=='agregar persona'){
	...
	if(strlen($_POST['apellido_pat'])<1 || strlen($_POST['nombre'])<1){
		...
		echo "<BR>";
		echo "            <table align=center class=table_border width=40% border=0 cellpadding=3 cellspacing=0>\n";
		echo "              <tr><td class=table_rows align=center colspan=3 style='color:red;font-family:Tahoma;font-size:12px;'>ERROR: alguno de los valores no es valido !!!</td></tr>\n";
		echo "            </table>\n";
		...
		$result = mysql_query($sql);
			echo "<BR>";
			echo "            <table align=center class=table_border width=40% border=0 cellpadding=3 cellspacing=0>\n";
			echo "              <tr><td class=table_rows align=center colspan=3 style='color:green;font-family:Tahoma;font-size:12px;'>La persona se agrego con exito !!!</td></tr>\n";
			echo "            </table>\n";
	}
}
...
if($_POST['submit']=='borrar persona'){
	...
	$result = mysql_query($sql);
		echo "<BR>";
		echo "            <table align=center class=table_border width=40% border=0 cellpadding=3 cellspacing=0>\n";
		echo "              <tr><td class=table_rows align=center colspan=3 style='color:red;font-family:Tahoma;font-size:12px;'>La persona se borro con exito !!!</td></tr>\n";
		echo "            </table>\n";
...
echo "            <br />\n";
echo "            <form name='libro' action='$current_page' method='post'>\n";
echo "<table align=center class=table_border><form method='post' action'addpersona.php'>";
$sql = "SELECT * FROM personas ORDER BY apellido_pat, apellido_mat, nombres";
$result = mysql_query($sql);
echo '<tr><td><STRONG>VERIFICADOR</STRONG></td><td><select id="persona" acdropdown=true style="width:450px;" autocomplete_complete="false" autocomplete_onselect="alertSelected" name="id">';
while($persona = mysql_fetch_array($result)){
	echo "<option value='".$persona['id']."'>".$persona['apellido_pat']." ".$persona['apellido_mat']." ".$persona['nombres']."</option>\n";
}
echo "</select></td><td><input type='submit' name='submit' src='images/icons/delete.png' value='borrar persona'>
