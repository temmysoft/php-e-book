...
if($_POST['submit']=='cambiar'){
	...
	if($error==0){
		...
		$sql = "UPDATE users SET nombre ='".$_POST['nombre']."',  pass=encode('".$_POST['password']."','xy'), user='".$_POST['user']."', admin='".$admin."', capturista='".$captur."', lector='".$lector."', inicio='".$_POST['fecha_inicio']."', fin='".$_POST['fecha_fin']."' WHERE id='".$_POST['id']."'";
		$res = mysql_query($sql);
		echo "<BR>";
		echo "            <table align=center class=table_border width=40% border=0 cellpadding=3 cellspacing=0>\n";
...
if($_POST['submit']=='agregar usuario'){
	...
	if($error == 0){ // realizar inserts, todo correcto
		...
		$sql = "INSERT INTO users (nombre, user, pass, admin, capturista, lector, inicio, fin) VALUES (
				'".$_POST['nombre']."',
				'".$user."',
				encode('".$pass."','xy'),
				'".$admin."',
